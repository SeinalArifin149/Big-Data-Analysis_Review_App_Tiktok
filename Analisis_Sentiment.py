...
df['Sentiment_Label'] = df['stop_removed'].apply(auto_label_sentiment)
...
# ==========================================
X_all_vec = vectorizer.transform(df['stop_removed'].astype(str))
...
ax = sns.countplot(
    data=df, 
    x='Topik_ID', # Ganti jadi 'Label_Topik' kalau mau nama topik yang muncul
    hue='Sentiment_NB', 
...
print("\n=== RINGKASAN DATA ===")
summary = df.groupby(['Topik_ID', 'Sentiment_NB']).size().unstack(fill_value=0)
print(summary)